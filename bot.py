...
BOT_TOKEN = (os.getenv("8784120583:AAHftJDUue1gYvCPRfKeC7fMuDfT9PMhk2E") or "").strip()
ADMIN_ID = int(os.getenv("ADMIN_ID", "71031452"))
DB_PATH = os.getenv("DB_PATH", "users.db")

print(f"🔍 Checking ENV... ADMIN_ID={ADMIN_ID}, PYTHON_VERSION={os.getenv('PYTHON_VERSION')}")
env_keys = [k for k in os.environ.keys() if 'TOKEN' in k or 'ADMIN' in k or 'BOT' in k]
print(f"🔍 ENV keys: {env_keys}")
raw_token = os.getenv("8784120583:AAHftJDUue1gYvCPRfKeC7fMuDfT9PMhk2E", "")
print(f"🔍 BOT_TOKEN raw length: {len(raw_token)} | stripped length: {len(BOT_TOKEN)} | repr: {repr(raw_token[:20])}")

if not BOT_TOKEN:
    print("❌ BOT_TOKEN خالیه یا فقط فاصله است!")
    print("👉 برو Render > Environment > BOT_TOKEN رو پاک کن و دوباره با توکن واقعی بساز")
    raise ValueError("BOT_TOKEN خالیه - مقدارش رو درست Paste کن")
